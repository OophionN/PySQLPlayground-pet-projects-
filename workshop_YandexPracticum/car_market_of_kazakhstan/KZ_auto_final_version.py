...
# а можно еще аннотирование добавить ))))

def data_preprocessing(data: pd.core.frame.DataFrame) -> pd.core.frame.DataFrame:
    data.columns = data.columns.str.lower().str.replace(' ', '_')
    data = data.apply(lambda x: x.str.lower() if x.dtype == "object" else x)
    return data
...
# Удалось заполнить только 1032 (пропуски и нули) значений, что тоже не плохо, мы снизили с 5.96 до 3,4 пропуски, остальное можем срезать
...
# Получилось немного заполнить пропуски в разделе топливо, из 739 (2,33%) изначальных, осталось 289 (0,91%), остальное срезали. 
...
# ### Итоги чистки
# 
# После всех чисток и замен, из 32854 изначальных строк, осталось 30504, или 93%. Приемлемо. Идем дальше
...
# Учитывая датасет с которым подошли к этапу анализа, пришло время посмотреть на числовые значения, в том числе на выбросы
